...
const Register = () => {
  ...
  const [modalUsn, setModalUsn] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Pastikan data.usn dan data.email selalu ada
      if (!data.usn && email) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f8f3ec] px-2">
      <Modal
        ...
        icon={
          <svg width="40" height="40" fill="none" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="12" fill="#f7b267" />
            <path
              d="M7 13l3 3 7-7"
              stroke="#fff"
              strokeWidth="2.5"
      ...
      >
        <div>
          Username kamu:{" "}
          <span className="font-bold text-orange-500">{modalUsn}</span>
          <br />
          Selamat bergabung di CivicLearn!
        </div>
      </Modal>
      <motion.div
      ...
      >
        ...
        <p className="text-center text-gray-400 mb-1 text-xs">
          Daftar untuk mulai belajar hukum!
        </p>
        {error && (
          <div
            ref={errorRef}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {/* Input nama dihapus, hanya email & password */}
